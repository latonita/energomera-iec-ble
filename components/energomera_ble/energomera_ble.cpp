...
static const uint16_t ENERGOMERA_SERVICE_UUID_16 = 0x0100;
static const uint16_t ENERGOMERA_TX_UUID_16 = 0x0105;

EnergomeraBleComponent::EnergomeraBleComponent() : PollingComponent(10 * 1000) {}
...
void EnergomeraBleComponent::loop() {
  ...
  if (this->state_deadline_ != 0 && now > this->state_deadline_) {
    ESP_LOGW(TAG, "State %d timed out", static_cast<int>(this->state_));
    this->reset_with_backoff_("timeout");
    return;
  }
  ...
  switch (this->state_) {
    ...
    case ClientState::ENABLING_NOTIFICATIONS:
...
void EnergomeraBleComponent::update() {
  if (this->state_ != ClientState::READY) {
    ESP_LOGV(TAG, "Update skipped, client not ready");
    return;
  }
  // Placeholder for application-specific polling such as requesting watch data.
}
...
void EnergomeraBleComponent::change_state_(ClientState next_state) {
...
void EnergomeraBleComponent::enable_notifications_() {
  ESP_LOGI(TAG, "Enabling TX notifications");
  esp_ble_gattc_register_for_notify(this->parent_->get_gattc_if(), this->target_address_.data(), this->tx_char_handle_);
  if (this->notify_cccd_handle_ != 0) {
    uint8_t notify_en[2] = {0x01, 0x00};
    esp_ble_gattc_write_char_descr(this->parent_->get_gattc_if(), this->parent_->get_conn_id(),
                                   this->notify_cccd_handle_, sizeof(notify_en), notify_en, ESP_GATT_WRITE_TYPE_RSP,
...
void EnergomeraBleComponent::sync_address_from_parent_() {
  ...
  if (!nonzero)
    return;
  memcpy(this->target_address_.data(), remote, 6);
  this->address_set_ = true;
  ESP_LOGD(TAG, "Using parent BLE address: %02X:%02X:%02X:%02X:%02X:%02X", remote[0], remote[1], remote[2], remote[3],
           remote[4], remote[5]);
}
void EnergomeraBleComponent::reset_with_backoff_(const char *reason) {
  if (reason != nullptr) {
    ESP_LOGW(TAG, "Resetting client: %s", reason);
  }
  this->retry_count_++;
  if (this->retry_count_ > kMaxRetries) {
    ESP_LOGE(TAG, "Maximum retries reached");
    this->change_state_(ClientState::ERROR);
    this->mark_failed();
    return;
  }
  this->stop_scan_();
...
bool EnergomeraBleComponent::match_service_uuid_(const esp_bt_uuid_t &uuid) const {
  if (uuid.len == ESP_UUID_LEN_128) {
    // 128-bit UUIDs must match full string
    static const uint8_t svc_uuid[16] = {0xdf, 0x14, 0xd9, 0x62, 0xd8, 0x1c, 0xc3, 0x97,
                                         0xe2, 0x45, 0xef, 0x8b, 0x01, 0x01, 0x1b, 0xb9};
    return memcmp(uuid.uuid.uuid128, svc_uuid, sizeof(svc_uuid)) == 0;
  }
  if (uuid.len == ESP_UUID_LEN_16) {
    return uuid.uuid.uuid16 == ENERGOMERA_SERVICE_UUID_16;
  }
  return false;
}

bool EnergomeraBleComponent::match_characteristic_uuid_(const esp_bt_uuid_t &uuid, const esp_bt_uuid_t &target) const {
  if (uuid.len != target.len)
    return false;
  if (uuid.len == ESP_UUID_LEN_16)
    return uuid.uuid.uuid16 == target.uuid.uuid16;
  if (uuid.len == ESP_UUID_LEN_32)
    return uuid.uuid.uuid32 == target.uuid.uuid32;
  return memcmp(uuid.uuid.uuid128, target.uuid.uuid128, sizeof(uuid.uuid.uuid128)) == 0;
}
